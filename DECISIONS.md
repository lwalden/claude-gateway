...
### /metrics + static status dashboard | 2026-07-25 | Status: Active
Chose: a `GET /metrics` endpoint that aggregates the last 24h of traffic by re-reading today's + yesterday's JSON-lines log files on each request, plus a static dashboard (`src/public/index.html`, served via `express.static` at `/`) that polls `/health`, `/health/cli`, and `/metrics` client-side. Over: (a) an in-memory counter — rejected, resets on every restart and undercounts on multi-process deployments; (b) a real metrics store (Prometheus, sqlite) — rejected as overkill for a single-user local gateway with a `logs/` directory already rotating daily. `/metrics` requires the same `GATEWAY_API_KEY` bearer token as `/ask` (matches existing auth posture); `/health` stays open so the dashboard can distinguish "offline" (fetch fails) from "unhealthy" (reachable but degraded — CLI auth expired or 24h success rate `< 80%`) without a key. The API key is entered once in the browser and kept in `localStorage`, never sent anywhere but the gateway itself. Tradeoff: re-parsing log files per request is O(day's log size) rather than O(1); acceptable at this traffic volume. Consequence: `/health` and `/health/cli` calls are excluded from the metrics counts (they're liveness pings, not gateway traffic).

### CLAUDE_OAUTH_CLIENT_ID direct refresh — ToS risk, left unset | 2026-07-25 | Status: Revisit
Chose: leave `CLAUDE_OAUTH_CLIENT_ID` unset and treat `auth-refresh.js`'s direct `refresh_token` grant against `platform.claude.com` as dormant, rather than configuring it with the Claude Code CLI's own OAuth client_id (`9d1c250a-e61b-44d9-88ed-5944d1962f5e` — findable via community write-ups and a GitHub issue on `anthropics/claude-code`, but not an Anthropic-published integration point). Why: community-sourced reporting (not independently verified against Anthropic's ToS text) describes a February 2026 Consumer Terms of Service update restricting subscription (Pro/Max) OAuth tokens to use within Claude Code and Claude.ai only. A third-party tool performing its own `refresh_token` grant against the OAuth endpoint, bypassing the CLI entirely, plausibly falls outside that — unlike the rest of the gateway, which only ever shells out to the user's already-signed-in `claude` CLI. Tradeoff: no proactive token refresh; the CLI's own native session handling is relied on instead, and `/health/cli` must be checked periodically (e.g. via the dashboard) to catch an `expired` token. Alternatives considered: (a) configure it anyway since the client_id is technically obtainable — rejected, not worth the ToS risk to the account for a convenience feature; (b) remove `auth-refresh.js` entirely — deferred, since it's inert without the env var rather than actively risky; revisit if Anthropic publishes an official statement, or replace with a supported mechanism (`claude setup-token` → long-lived `CLAUDE_CODE_OAUTH_TOKEN`) if evaluated favorably.
...
|---|---|---|---|---|
| D-001 | `auth-refresh.js` performs a direct `refresh_token` grant against `platform.claude.com` using the Claude Code CLI's own client_id — plausible ToS conflict per community reporting on the Feb 2026 Consumer ToS update restricting subscription OAuth tokens to Claude Code/Claude.ai. Currently dormant (`CLAUDE_OAUTH_CLIENT_ID` unset). | Feature is inert; if ever configured, risks the account's ToS standing | 2026-07-25 | - |
